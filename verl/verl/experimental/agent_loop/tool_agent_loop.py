...
def truncate_at_call_tool(output, tokenizer):
    target_str = "</google_search>"
    ...
    for i in range(len(ids)):
        # 将从开头到当前位置的 tokens 解码
        # 这种方式可以规避分词歧义，因为 decode(ids[:i]) 是唯一的
        current_decoded = tokenizer.decode(ids[:i+1])
        
        if target_str in current_decoded:
            # 找到第一个包含该字符串的 token 位置
            found_idx = i
            break
    ...
    # 截断位置就是 found_idx + 1 (包含当前这个 token)
    end_pos = found_idx + 1
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # Finalize output
        response_ids = agent_data.prompt_ids[-len(agent_data.response_mask) :]  # prompt_ids的后半部分为response_ids
    ...
    async def _handle_generating_state(
        self, agent_data: AgentData, sampling_params: dict[str, Any], ignore_termination: bool = False) -> AgentState:
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        response_ids = response_ids[len(self.system_prompt) :]  # 保存了多次工具调用的id
        if len(agent_data.response_mask) + len(response_ids) >= self.response_length:
            return AgentState.TERMINATED
        # Update prompt_ids and response_mask

        # 这里很关键
        agent_data.prompt_ids += response_ids  # 将tool_response_id拼接到prompt_ids后边
    ...
    async def _handle_interacting_state(self, agent_data: AgentData) -> AgentState:
        # print("_handle_interacting_state_handle_interacting_state_handle_interacting_state")
