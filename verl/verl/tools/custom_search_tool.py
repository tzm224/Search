...
# 导入search工具
# 用法
# text = search(query_list, top_k=5)
from verl.tools.search_utils.tools import search
...
class CustomSearchTool(BaseTool):
    ...
    def execute_search(self, instance_id: str, query_list: list, topk: int, timeout: int):
        ...
        """
        result_text = search(query_list, top_k=topk)
        metadata = {
            "query_count": 1,
            "status": "success" if "snippet" in result_text else "error",
            "total_results": 5,
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        try:
            result_text, metadata = await self.execution_pool.execute.remote(
                self.execute_search, instance_id, query_list_from_params, self.topk, self.timeout
            )
            ...
            logger.error(f"[CsutomSearchTool] Execution failed: {e}, query_list: {query_list_from_params}")
