    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # print("一开始batch含有的keys：", batch.non_tensor_batch.keys())  # 完整的keys
                ...
                # print("一开始gen_batch含有的keys：", gen_batch.non_tensor_batch.keys())  # 完整的keys - 去掉的keys
                # print("gen_batch生成后batch含有的keys：", batch.non_tensor_batch.keys()) # 只有dict_keys(['extra_info', 'uid'])

                # pass global_steps to trace
                gen_batch.meta_info["global_steps"] = self.global_steps
                gen_batch = gen_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                ori_non_tensor_batch = gen_batch.non_tensor_batch

                # print("gen_batch重复后含有的keys：", gen_batch.non_tensor_batch.keys(), gen_batch.non_tensor_batch["extra_info"])
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # update gen_bath_output
                    # print("rollout结束后含有的keys：", gen_batch.non_tensor_batch.keys(), gen_batch.non_tensor_batch["extra_info"])
                    gen_batch_output.non_tensor_batch.update(ori_non_tensor_batch)
                    ...
                    # print("batch重复后含有的keys：", batch.non_tensor_batch.keys(), batch.non_tensor_batch["extra_info"])
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, acc_rewards, format_rewards, search_rewards, search_nums = self.reward_fn(batch, self.global_steps)
                            reward_extra_infos_dict = {}
                    
                    # update metrics
                    reward_metrics = {
                        "acc/rewards/mean": np.mean(acc_rewards),
                        "acc/rewards/var": np.var(acc_rewards),
                        "acc/rewards/max": np.max(acc_rewards),
                        "acc/rewards/min": np.min(acc_rewards),

                        "search/rewards/mean": np.mean(search_rewards),
                        "search/rewards/var": np.var(search_rewards),
                        "search/rewards/max": np.max(search_rewards),
                        "search/rewards/min": np.min(search_rewards),

                        "search/nums/mean": np.mean(search_nums),
                        "search/nums/var": np.var(search_nums),
                        "search/nums/max": np.max(search_nums),
                        "search/nums/min": np.min(search_nums),
